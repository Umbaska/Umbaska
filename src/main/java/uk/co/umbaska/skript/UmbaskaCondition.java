...
public abstract class UmbaskaCondition extends Condition implements AutoRegisteringSkriptElement, ExpressionManagerProvider, DynamicSyntax{
    ...
    public String toString(Event event, boolean b) {
        return String.format("Umbaska Condition %s", getClass().getCanonicalName());
    }
            ...
            if (method.getDeclaringClass().equals(getClass())) {
                switch (method.getName()) {
                    case "check":
                        return check();
                    case "checkWithEvent":
                        return checkWithEvent(new EventSource(event));
                }
            }
        }
    ...
    public boolean check() { throw new UnsupportedOperationException();}
    public boolean checkWithEvent(EventSource eventSource) { throw new UnsupportedOperationException();}
